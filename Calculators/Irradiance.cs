...
namespace sun_forecast_calculation_app.Calculator
{
    class Irradiance : IIrradiance
    {
        ...
        {
            sunIrradiance.dni = Math.Round(dni_extra * Math.Pow(transmittens, airmass));
            sunIrradiance.dhi = Math.Round(0.3 * (1.0 - Math.Pow(transmittens,airmass)) * dni_extra * sunZenith);
            sunIrradiance.ghi = Math.Round(sunIrradiance.dhi + sunIrradiance.dni * sunZenith);
